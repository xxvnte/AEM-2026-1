# python3 main.py case1.txt 1
# python3 main.py case2.txt 1
# python3 main.py case3.txt 1
# python3 main.py case4.txt 1
...
def parse_instance(filepath):
    ...
    for k in range(D):
        ...
        L = float(tokens[idx])
        idx += 1
        c_e = float(tokens[idx])
        idx += 1
        c_l = float(tokens[idx])
        idx += 1
...
def backtracking(D, planes, sep, num_runways, domains, time_limit):
    best = {"cost": float("inf"), "assignment": None, "runway": None}
    nodes_explored = [0]
    time_exceeded = [False]
...
def print_solution(
    D, planes, assignment, runway_of, cost_val, nodes, elapsed, time_exceeded
):
    print("\n" + "=" * 60)
    ...
    status = (
        ...
        else " (MEJOR ENCONTRADA — límite de tiempo alcanzado)"
    )
...
def main():
    ...
    filepath = resolve_case_path(filepath)
    ...
    print_solution(
        D, planes, assignment, runway_of, best_cost, nodes, elapsed, time_exceeded
    )
