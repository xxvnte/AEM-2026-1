...
def parse_instance(filepath):
    ...
    # FORMATO: para cada avión → E P L c_e c_l, luego D valores de separación
    for k in range(D):
...
# ─────────────────────────────────────────
# 6. BACKTRACKING CRONOLÓGICO
# ─────────────────────────────────────────

def backtracking(D, planes, sep, num_runways, domains):
    best = {
        'cost': float('inf'),
        'assignment': None,
        'runway': None
    ...
    bt({}, 0, 0.0)
    return best['assignment'], best['runway'], best['cost'], nodes_explored[0]
...
def print_solution(D, planes, assignment, runway_of, cost_val, nodes, elapsed):
    print("\n" + "="*60)
    ...
    if assignment is None:
        ...
        return
    print(f"  Costo total       : {cost_val:.2f}")
...
def main():
    ...
    t0 = time.time()
    assignment, runway_of, best_cost, nodes = backtracking(
        D, planes, sep, num_runways, copy.deepcopy(domains)
    )
    elapsed = time.time() - t0

    print_solution(D, planes, assignment, runway_of, best_cost, nodes, elapsed)
